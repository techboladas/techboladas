...
export const NavBar = () => {
  ...
  return (
    <nav
      className={`${width} h-screen bg-tech-secondary   text-2xl text-white fixed  grid ${grid} auto-rows-max  transition-all`}
    >
      <section
        className={`${display} flex-col gap-10 text-center w-48 mt-28  justify-self-end  col-span-2`}
      >
        <Link to="apresentation" smooth duration={500}>
          Apresentacao
        </Link>
        <Link to="service" smooth duration={500}>
          Servicos
        </Link>
        <Link to="catalog" smooth duration={500}>
          Catalogo
        </Link>
      ...
      <Menu className="col-span-1">
        <MenuItem
          className=" self-start justify-self-center w-full h-full border-none "
          onClick={handleClick}
        >
        ...
        <MenuItem>
          <A
            href="https://www.facebook.com/profile.php?id=61553282626827&mibextid=3JLxgoXK8XkhKqoE"
            target="_blank"
          >
